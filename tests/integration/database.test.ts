...
describe("RuntimeDatabase", () => {
  it("applies migrations once and persists data across reopen", async () => {
    ...
    expect(first.all<{ version: number }>("SELECT version FROM schema_migrations")).toEqual([{ version: 1 }, { version: 2 }]);
    ...
    expect(reopened.all("SELECT version FROM schema_migrations")).toHaveLength(2);
    reopened.close();
  });
