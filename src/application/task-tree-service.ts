  ...
  private persistRevision(tree: TreeRow, projectId: string, document: TaskTreeDocument, wrap = true): TaskTreeRevisionView {
    ...
    const work = () => {
      ...
      for (const artifact of document.artifacts) {
        this.database.run(
          "INSERT INTO artifacts (id, project_id, tree_id, kind, locator, status, metadata_json, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, '{}', ?, ?) ON CONFLICT(project_id, kind, locator) DO UPDATE SET tree_id = excluded.tree_id, status = excluded.status, updated_at = excluded.updated_at",
          artifact.id, projectId, tree.id, artifact.kind, artifact.locator, artifact.status ?? "draft", timestamp, timestamp,
        );
      }
